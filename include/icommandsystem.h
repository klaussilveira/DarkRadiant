...
class Argument
{
    ...
    int _intValue = 0;
    Vector3 _vector3Value = {0, 0, 0};
    Vector2 _vector2Value = {0, 0};

    // The type flags
    std::size_t _type = ARGTYPE_VOID;
...
private:
    void tryNumberConversion()
    {
        ...
        int maybeInt;
        if (string::tryConvertToInt(_strValue, maybeInt))
        {
            _intValue = maybeInt;
            _type |= ARGTYPE_INT;
        }
        ...
        double maybeDouble;
        if (string::tryConvertToDouble(_strValue, maybeDouble))
        {
            _doubleValue = maybeDouble;
            _type |= ARGTYPE_DOUBLE;
        }
    }

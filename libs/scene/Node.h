...
private:
    ...
    mutable bool _boundsChanged = true;
    mutable bool _boundsMutex = false;
    mutable bool _childBoundsChanged = true;
    mutable bool _childBoundsMutex = false;
    mutable bool _transformChanged = true;
    mutable bool _transformMutex = false;

    mutable Matrix4 _local2world = Matrix4::getIdentity();
    ...
    RenderState _renderState = RenderState::Active;

protected:
    // If this node is attached to a parent entity, this is the reference to it
    IRenderEntity* _renderEntity = nullptr;

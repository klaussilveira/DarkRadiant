...
public:
    ...
    /// Construct a BasicVector3 out of 2 elements of type OtherType
    ...
    /// Access the vector elements as a C array
    const Element* data() const { return _v; }

    /// Access const element by index (no bounds checking)
    const Element& operator[] (std::size_t index) const
    {
        return _v[index];
    }

    /// Access non-const element by index (no bounds checking)

...
The mechanism for hiding assets depends on the asset type: models and materials
use a file-based `assets.lst` approach, whereas entity classes use a dedicated
spawnarg.

==== Hiding models or materials

To change the visibility of models or materials, you need to create a file
called `assets.lst` which resides in the top-level `models` or `materials`
directory in the mod installation. This file may be inside a PK4 or in an
